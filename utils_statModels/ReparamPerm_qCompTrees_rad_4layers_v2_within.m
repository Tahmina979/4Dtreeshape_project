...
function [G,qCompTree_4Ls1p, qCompTree_4Ls2] = ReparamPerm_qCompTrees_rad_4layers_v2_within(qCompTree_4Ls1,qCompTree_4Ls2, lam_m,lam_s,lam_p)

% Pad minor trees
[qCompTree_4Ls1, qCompTree_4Ls2] = CompatMax_rad_4layers_within(qCompTree_4Ls1, qCompTree_4Ls2);
...
if(K1~=K2)
    fprintf('not a growing structure');
end
...
for i=1:K1
    ...
    for j=1:K2
        ...
        [G, ~] = ReparamPerm_qCompTrees_rad_3layers_v2_within(qCompTree_4Ls1.q_children{i}, ...
...
for i = 1: numel(qCompTree_4Ls1p.q_children)
   ...
   [~, qCompTree_4Ls1p.q_children{i}, qCompTree_4Ls2.q_children{i}] = ...
                           ReparamPerm_qCompTrees_rad_3layers_v2_within(qCompTree_4Ls1p.q_children{i}, qCompTree_4Ls2.q_children{i}, ...
                                                                lam_m,lam_s,lam_p);
   %if qCompTree_4Ls1p.q_children{i}.K_sideNum==qCompTree_4Ls2.q_children{i}.K_sideNum
