...
function [G,qST1p, qST2] = ReparamPerm_qST_wg_rad(qST1,qST2, lam_m,lam_s,lam_p)
...
%     qST1p.q_children{unmatched2(i)}.func_t_parasAndRad =[];
...
% --- here I think the tk1p should not be changed ---
tk1p = qST1.tk_sideLocs;
...
if isfield(qST1p, 'q_children') == 0
    qST1p = orderfields(qST1p, ...
        {'t_paras','s','q0','T0_pointNum','len0','K_sideNum','q','T_sidePointNums','len','tk_sideLocs','sk','dimension','b00_startP', 'func_t_parasAndRad', 'data_t_parasAndRad'});
else
    qST1p = orderfields(qST1p, ...
