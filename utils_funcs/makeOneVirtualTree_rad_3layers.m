...
q_zero.beta_rad = q_children.beta_rad;

q_zero.q_children = cell(1, 0);
for i = 1: numel(q_zero.q)
    q_zero.q{i} = zeros(size(q_children.q{i}, 1), size(q_children.q{i}, 2));
    q_zero.q_children{i} = makeZeroST_rad(q_children.q_children{i});
end
