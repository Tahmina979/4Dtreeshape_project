...
function [G,qCompTree_3Ls1p, qCompTree_3Ls2p] = ReparamPerm_qCompTrees_3layers(qCompTree_3Ls1,qCompTree_3Ls2, lam_m,lam_s,lam_p)
...
for i=1:K1
    ...
    q_zero = makeZeroST(qCompTree_3Ls1.q_children{i});
    [G1] = ReparamPerm_qST_forZero(qCompTree_3Ls1.q_children{i}, q_zero, lam_m,lam_s,lam_p);
    E(i,K2+1:end) = lam_s* sqrt(G1.E);
end

for j=1:K2
    clear q_zero
    q_zero = makeZeroST(qCompTree_3Ls2.q_children{j});
    [G2] = ReparamPerm_qST_forZero(q_zero, qCompTree_3Ls2.q_children{j}, lam_m,lam_s,lam_p);
...
qCompTree_3Ls2p.q = cell(1,K);

for i=1:m
    
    qCompTree_3Ls1p.q{matched(i,2)} = GammaActionQ(qCompTree_3Ls1.q{matched(i,1)}, G.gam{i});
    qCompTree_3Ls1p.q_children{matched(i,2)} = qCompTree_3Ls1.q_children{matched(i,1)};
    
    qCompTree_3Ls1p.q_children{matched(i,2)}.q0 = qCompTree_3Ls1p.q{matched(i,2)};
%     qCompTree_3Ls2p.q{matched(i,2)} = qCompTree_3Ls2.q{matched(i,2)};
%     qCompTree_3Ls1p.q_children{matched(i,2)} = qCompTree_3Ls1.q_children{matched(i,2)};
end

for i=1:um2
    
    qCompTree_3Ls1p.q{unmatched2(i)} = zeros(qCompTree_3Ls1.dimension, qCompTree_3Ls2.T_sidePointNums(unmatched2(i)));
    qCompTree_3Ls1p.q_children{unmatched2(i)} = makeZeroST(qCompTree_3Ls2.q_children{unmatched2(i)});
    qCompTree_3Ls1p.q_children{unmatched2(i)}.q0 = qCompTree_3Ls1p.q{unmatched2(i)};
end

for i=1:um1
    
    qCompTree_3Ls1p.q{m+um2+i} = qCompTree_3Ls1.q{unmatched1(i)};
    qCompTree_3Ls1p.q_children{m+um2+i} = qCompTree_3Ls1.q_children{unmatched1(i)};
    qCompTree_3Ls1p.q_children{m+um2+i}.q0 = qCompTree_3Ls1p.q{m+um2+i};
...
for i=1:um1
    qCompTree_3Ls1p.tk_sideLocs(m+um2+i) = tk1p(unmatched1(i));
    qCompTree_3Ls1p.sk(m+um2+i) = sk1p(unmatched1(i));
end

% unmatched2 get arclength position from tree 2
qCompTree_3Ls1p.sk(unmatched2) = s2k(unmatched2);
qCompTree_3Ls1p.tk_sideLocs(unmatched2) = interp1(qCompTree_3Ls1p.s, qCompTree_3Ls1p.t_paras, qCompTree_3Ls1p.sk(unmatched2));
