function [Mu,eigenVector, EVals,tree_pca,used_qCompTrees_Ready]=transfer_to_PCA_space(ctree_1,ctree_2,ctree_3,ctree_4,ctree_5,ctree_6,ctree_7,num_within_seq1,num_within_seq2,num_within_seq3,num_within_seq4,num_within_seq5,num_within_seq6,num_within_seq7)
lam_m = 1; 
lam_s = 1;
lam_p = 1;
...
for i=1:num_within_seq1
  ...
  used_qCompTrees{i}=ctree_1.aligned_q_tree_1{1,i};
end
...
for i=p:p_end
  ...
  used_qCompTrees{i}=ctree_2.aligned_q_tree_2{1,i-p+1};

end
p=p_end+1; p_end=p_end+num_within_seq3;
for i=p:p_end
    used_qCompTrees{i}=ctree_3.aligned_q_tree_3{i-p+1};
end
p=p_end+1; p_end=p_end+num_within_seq4;
for i=p:p_end

    used_qCompTrees{i}=ctree_4.aligned_q_tree_4{i-p+1};
end
p=p_end+1; p_end=p_end+num_within_seq5;
for i=p:p_end

    used_qCompTrees{i}=ctree_5.aligned_q_tree_5{i-p+1};
end

p=p_end+1; p_end=p_end+num_within_seq6;
for i=p:p_end
  used_qCompTrees{i}=ctree_6.aligned_q_tree_6{i-p+1};
end
p=p_end+1; p_end=p_end+num_within_seq7;
for i=p:p_end

    used_qCompTrees{i}=ctree_7.aligned_q_tree_7{i-p+1};
end
